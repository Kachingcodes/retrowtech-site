...
const Navbar = () => {
  ...
  const scrollSections = ['Home', 'Customers', 'Services', 'Client Feedback'];
      ...
      <header className="fixed top-0 left-0 w-full z-50 bg-[#181c25]/90 backdrop-blur-md text-white">
        <nav className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <ul className="hidden md:flex gap-8 text-sm font-medium items-center">
            ...
            <li>
              <Link href="/about" className="hover:text-[#FE0628]">About</Link>
            </li>
            <li>
              <button
        ...
        <AnimatePresence>
          {open && (
            ...
            {scrollSections.map((section) => (
              <div key={section} className="border-b border-[#bcc8d6] pb-3">
                <ScrollLink
                  to={section}
                  spy={true}
                  smooth={true}
                  offset={-100}
                  duration={500}
                  className="text-lg font-medium px-6 text-white hover:text-[#FE0628] transition cursor-pointer"
              ...
              <Link
                href="/about"
                className="text-lg font-medium px-6 text-white hover:text-[#FE0628] transition border-b border-[#bcc8d6] pb-3"
                onClick={() => setOpen(false)}
              >
                About
              </Link>
